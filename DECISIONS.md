...
## 2026-07-11 — Figma is now the visual source of truth for the dashboard (supersedes the HTML prototypes)

**Decision**: Mid-M8, the user shared a Figma file (`pPXy7Z1Sp0LG4G4FEmbray`) with a more current, more precise dashboard design than `DESIGN.md`'s prose + the `morch_prototype_branded`/`morch_prototype_ux_fixes` HTML prototypes `IMPLEMENTATION_PLAN.md`'s M8 section pointed at. Pulled exact specs (colors, spacing, font sizes, copy) via Figma's MCP tools and reworked the dashboard's header, toolbar, sidebar, instruction rows, board columns, and both drawers to match — notably: toolbar sits on `--background` (not `--surface`) with a `--border-strong` separator; sidebar items always carry a leading file/folder icon (swapped to the warning triangle when flagged); instruction rows use a `--rounded-card: 8px` corner radius not in DESIGN.md's documented scale; board columns are 260px with a two-line header (name, then "X/Y active" below); the disabled-archive drawer's top border is `--border-strong` while the ignored-files drawer's is the regular `--border`.

Two explicit exclusions, confirmed with the user: (1) Figma's header shows 3 small dots styled like macOS traffic lights, but colored with Morch's own Aurora status tokens rather than literal stoplight colors, with no interactive state shown — the user said not to implement them at all, decorative or functional, so `tauri.conf.json` keeps native window decorations and no dots were added. (2) Figma's outer "app" frame carries a border/rounded-corner/shadow — that's Figma simulating a floating window on its own infinite canvas; the real Tauri window already provides that frame natively, so `.morch-app` deliberately has none of that.
