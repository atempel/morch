...
## 2026-09-07 — M10 QA pass satisfied with automated backend tests; M9's manual dashboard verification remains open

**Decision**: `docs/IMPLEMENTATION_PLAN.md` M10's two SPEC.md §9 checks are now automated, in `src-tauri/src/instructions.rs`'s test module: `disabled_instruction_is_genuinely_absent_from_the_file_an_ai_would_read` (a disabled instruction's content is verified absent from the active file — Core Principle #3 — while still present in the archive per SPEC.md §3.6) and `toggling_every_instruction_off_and_on_fifty_times_causes_zero_data_loss` (50 full off/on cycles across every instruction in a file, asserting the file is empty mid-cycle and the sorted line set matches the original after each restore). Both were added directly as unit tests alongside `InstructionManager`'s existing test suite rather than as a new integration-test crate target, matching this project's established pattern of colocating tests with the module they exercise.

A third test, `external_edit_is_detected_and_a_fresh_load_reflects_the_new_content` (`src-tauri/src/lib.rs`), automates the *backend* half of M9's USER_FLOWS.md §4.3 scenario: an external process rewrites a managed file, `watcher::start_watching` reports it, and a fresh `InstructionManager::load` (mirroring the frontend's `reloadInstructions` on the `morch://external-change` event) reflects the new content. This is **not** a substitute for M9's actual acceptance bar ("that scenario passes manually at least three times in a row, no missed updates or duplicate rows") — that requires driving the real Tauri dashboard window, which this headless, display-less session cannot do. M9 (GitHub issue #5) is left open, flagged for Alexandre to run the manual pass; the backend test only establishes that the watcher-to-reload plumbing it depends on is sound.
...
## 2026-09-12 — Work is spikes on an issue board; build-spike PRs merge on a structural guard, everything else keeps the second-agent review

**Decision**: Units of work are spike briefs in `docs/plan/spikes/` (content, versioned with the code) whose state is a GitHub issue (open / closed / `in-progress`, `ready` / `blocked` derived from the dependency graph). A PR that names a `kind: build` spike merges itself when CI is green and `scripts/check-pr-scope.mjs` passes: it closes the spike's issue, edits none of `docs/SPEC.md`, `docs/PRD.md`, `docs/USER_FLOWS.md`, `DESIGN.md`, `BRAND.md`, appends to this file rather than rewriting it, updates its own brief, and touches nothing under `.github/workflows/`. The merge job then closes the issue itself and fails the run if the issue is still open. Any other PR is labelled `needs-review` with the guard's output and takes the second-agent review from `.claude/skills/github-workflow/SKILL.md`, unchanged. `kind: research` and `kind: decision` briefs are never run unattended: the session writes the options and stops with `needs-decision`. Milestones are `docs/ROADMAP.md`'s buckets (`next`, `later`, `maintenance`), not numbers, because M1–M10 are done and were never spikes.

**Rationale**: Phase One shipped through hand-merged milestone PRs, which works while a person is in the loop for every merge and stops working the moment sessions run on a schedule. The three failure modes a scheduled routine has — two sessions taking the same work, a merged change whose issue stays open so the board lies, and a decision quietly made by a session instead of a person — are each covered by something that runs on GitHub rather than something a prompt asks for (`dedupe.yml`, the asserted close in `auto-merge.yml`, and `kind: decision` as a hard stop). The review rule is narrowed, not dropped: a build spike is bounded by its brief and by the guard, which is the structural half of a review; the judgement half still applies to anything the guard cannot bound. `CLAUDE.md`'s scope guardrail is what made `kind: decision` a stop rather than a label — the three items in ROADMAP.md's Next are on the board as questions, and the routine cannot answer them.

**Status**: Locked in. Bootstrap: run the `board` workflow by hand once after merge to create the labels and the first four issues (SP-000 and the three decision briefs). What cannot be verified before that: workflows that run from the default branch (`workflow_run`, `pull_request_target`) are exercised by the first spike PR after this lands.
